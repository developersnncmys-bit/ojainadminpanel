...
export default function Customers() {
  ...
  const { items, loading } = useResource('customers')
  ...
  const columns = [
    ...
    { key: 'spent', header: 'Total Spent', render: (r) => <span className="font-semibold">₹{(r.spent || 0).toLocaleString('en-IN')}</span> },
  ...
  return (
    <div>
      <PageHeader title="Customers" subtitle={loading ? 'Loading…' : `${items.length} registered customers`} />
      <div className="mb-4">
        <SearchBox value={q} onChange={setQ} placeholder="Search customers, city…" />
      </div>
      <DataTable columns={columns} rows={rows} empty={loading ? 'Loading customers…' : 'No customers found.'} />
    </div>
  )
}
